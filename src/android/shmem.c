...
#define ASHV_KEY_SYMLINK_PATH "/data/local/tmp/ashv_key_%d"
...
static int libandroid_shm_unlink(const char *name) {
    size_t namelen;
    char *fname;
    ...
    namelen = strlen(name);
    fname = (char *) alloca(sizeof("/data/local/tmp/") - 1 + namelen + 1);
    memcpy(fname, "/data/local/tmp/", sizeof("/data/local/tmp/") - 1);
    memcpy(fname + sizeof("/data/local/tmp/") - 1, name, namelen + 1);

    return unlink(fname);
}

static int libandroid_shm_open(const char *name, int oflag, mode_t mode) {
    size_t namelen;
    char *fname;
    int fd;
...
#undef shm_unlink
int shm_unlink(const char *name) __attribute__((alias("libandroid_shm_unlink")));
#undef shm_open
int shm_open(const char *name, int oflag, mode_t mode) __attribute__((alias("libandroid_shm_open")));