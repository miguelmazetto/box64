...
notandroid(GO("libdl.so.2", libdl))
notandroid(GO("libm.so.6", libm))
...
notandroid(GO("libpng16.so.16", png16))
...
notandroid(GO("libfreetype.so.6", freetype))
GO("libbz2.so.1", bz2)
...
notandroid(GO("libmpg123.so.0", mpg123))
notandroid(GO("libgnutls.so.30", gnutls))
GO("libpcre.so.3", libpcre)
notandroid(GO("libcups.so.2", libcups))
//GO("d3dadapter9.so.1", d3dadapter9)
GO("libvulkan.so.1", vulkan)
GO("libvulkan.so", vulkan)
//GO("libwayland-client.so.0", waylandclient)
GO("libgbm.so.1", gbm)
notandroid(GO("libxml2.so.2", xml2))
notandroid(GO("libxslt.so.1", xslt))
notandroid(GO("libldap_r-2.4.so.2", ldapr))
notandroid(GO("liblber-2.4.so.2", lber))
//GO("libnsl.so.1", nsl)
